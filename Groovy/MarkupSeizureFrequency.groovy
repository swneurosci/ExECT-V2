new File(scriptParams.outputFile).withWriterAppend{ out ->
  doc.getAnnotations("Original markups").get("SeizureFrequency").each{
    anno ->
      def f = anno.getFeatures()
      String[] id =  doc.getFeatures().get("gate.SourceURL").split("/")
      out.writeLine(/${id[-1]},${anno.start()},${anno.end()}, ${f.get('CUI')},/+
      /"${f.get('string')}","${f.get('CUIPhrase')}",${f.get('FrequencyChange')},/+
      /${f.get('NumberOfSeizures')},${f.get('LowerNumberOfSeizures')},${f.get('UpperNumberOfSeizures')},/+
      ...
      /${f.get('Age')},${f.get('AgeLower')},${f.get('AgeUpper')},${f.get('AgeUnit')},${f.get('Certainty')},/+
      /${f.get('Negation')}/)
    }
  }
  //extracting markup annotatin via GATE - seizure frequency